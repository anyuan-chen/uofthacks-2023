...
export async function serializeNodeWithDialog(
  node: FrameNode,
  imports: string,
  hooks: string,
  ...
  const normalSerialization = await traverse(node, nodeWithId, imports, hooks, formMap);
  const dialogContent = await traverse(nodeWithId, nodeWithId, imports, hooks, formMap);
